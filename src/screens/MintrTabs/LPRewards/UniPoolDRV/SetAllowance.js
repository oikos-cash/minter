...
const SetAllowance = ({ t, goBack }) => {
	...
	const onUnlock = async () => {
		...
		try {
			...
			const gasEstimate = await uniswapDRVContract.estimateGas.approve(
				unipoolDRVContract.address,
				parseEther(ALLOWANCE_LIMIT.toString())
			);
			const transaction = await uniswapDRVContract.approve(
				unipoolDRVContract.address,
				parseEther(ALLOWANCE_LIMIT.toString()),
				{
	...
	return (
		<>
			...
			<TitleContainer>
				...
				<PLarge>{t('derivepool.locked.subtitle')}</PLarge>
			</TitleContainer>
			<ButtonRow>
