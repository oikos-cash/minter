...
export default withTranslation()(({ t }) => {
    const {  unipoolDRVContract, oldUnipoolDRVContract } = snxJSConnector;
	...
	useEffect(() => {
		...
		(async () => {
			console.log(oldUnipoolDRVContract)
			const res = await oldUnipoolDRVContract.balanceOf(currentWallet);
			if (res > 0) {
				setOldBalance(res);
				setIsMigrated(false);
			}
	...
	const migrate = async () => {
		try {
			...
			await oldUnipoolDRVContract.withdraw(oldBalance);
	...
	return (
		<ButtonRow>
			...
			>
				{t('lpRewards.shared.buttons.migrate')} {(oldBalance / 1e18)} lp
			</ButtonAction>
		</ButtonRow>
	);
});